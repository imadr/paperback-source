...
var _Sources = (() => {
  ...
  var parseMangaDetails = ($2, mangaId) => {
    const title = decode($2("h1.text-2xl").first().text().trim());
    const image = $2("img.w-full.rounded-xl").first().attr("src") ?? "";
    const author = $2("div:has(span:contains(Author:)) > a").first().text().trim() ?? "";
    const description = decode($2("h3:contains(Synopsis)").parent().parent().find("p").first().text().trim()) ?? "";
    const arrayTags = [];
    for (const tag of $2("dl div:contains(Genres:) a").toArray()) {
    ...
    const rawStatus = $2("div.flex.flex-wrap.gap-2 span.rounded-full").first().text().trim() ?? "";
    let status = "Ongoing";
  ...
  var parseChapters = ($2, mangaId) => {
    ...
    for (const chapter of $2(chapterListSelector).toArray()) {
      const title = $2("span.font-medium", chapter).first().text().trim() ?? "";
      const name = $2("span.text-brand-400", chapter).first().text().trim() ?? title;
      const chapterId = $2(chapter).attr("href")?.split("/").pop()?.split("?").shift() ?? "";
      if (!chapterId) continue;
      const chapMatch = name.match(/#(\d+)/);
      const chapNum = chapMatch ? Number(chapMatch[1]) : 0;
      const date = new Date($2("span.text-xs.text-slate-500", chapter).last().text().trim());
  ...
  var parseChapterDetails = ($2, mangaId, chapterId) => {
    const pages = [];
    for (const images of $2("img", "div#reader-all").toArray()) {
      const src = $2(images).attr("data-src")?.trim() || $2(images).attr("src")?.trim();
      if (!src) continue;
      pages.push(src);
    }
  ...
  var parseHomeSections = ($2, sectionCallback) => {
    ...
    for (const comic of $2("a.hot-item").toArray()) {
      const image = $2("img", comic).first().attr("src") ?? "";
      const title = $2("p.truncate", comic).first().text().trim() ?? "";
      const id = $2(comic).attr("href")?.split("/").pop()?.split("?").shift() ?? "";
      const subtitle = $2("p.text-\\[10px\\]", comic).first().text().trim() ?? "";
    ...
    for (const comic of $2("div.reveal-card").toArray()) {
      const image = $2("img.h-28", comic).first().attr("src") ?? "";
      const title = $2("a.line-clamp-2", comic).first().text().trim() ?? "";
      const id = $2("a.line-clamp-2", comic).first().attr("href")?.split("/").pop()?.split("?").shift() ?? "";
      const subtitle = $2("span.rounded.bg-ink-700", comic).first().text().trim() ?? "";
      if (!id || !title) continue;
  ...
  var parseViewMore = ($2) => {
    const comics = [];
    const collectedIds = [];
    for (const item of $2("div.comic-list-layout .grid > .group").toArray()) {
      const image = $2("img", item).first().attr("src") ?? "";
      const title = $2("a.block.text-sm.font-semibold", item).first().text().trim() ?? "";
      const id = $2("a.block.text-sm.font-semibold", item).first().attr("href")?.split("/").pop()?.split("?").shift() ?? "";
      const subtitle = $2("a.block.text-xs", item).last().text().trim() ?? "";
  ...
  var parseSearch = ($2) => {
    const comics = [];
    const collectedIds = [];
    for (const item of $2(".rc-cover").parent("a.group").toArray()) {
      const image = $2("img", item).first().attr("src") ?? "";
      const title = $2("p", item).first().text().trim() ?? "";
      const id = $2(item).attr("href")?.split("/").pop()?.split("?").shift() ?? "";
      if (!id || !title || collectedIds.includes(id)) continue;
      comics.push(App.createPartialSourceManga({
        mangaId: id,
        image,
  ...
  var hasNextPage = ($2) => {
    return $2("nav a[rel=next]").length > 0;
  };
  ...
  var ReadComicsOnline = class _ReadComicsOnline {
    ...
    async getHomePageSections(sectionCallback) {
      ...
      const popularItems = parseViewMore(load(popularResponse.data));
    ...
    async getSearchResults(query) {
      ...
      const metadata = hasNextPage($2) ? { page: page + 1 } : void 0;
      return App.createPagedResults({
        results: manga,
        metadata
      });
